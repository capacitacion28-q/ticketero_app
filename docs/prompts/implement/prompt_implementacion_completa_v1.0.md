...
Implementar el Sistema Ticketero completo siguiendo el plan de implementación mediante desarrollo iterativo paso a paso para:

**Objetivos de Implementación:**
- Ejecutar las 8 fases del plan en secuencia (Setup → Migraciones → Entities → DTOs → Repositories → Services → Controllers → Schedulers)
...
3. **Confirmar**: Solicitar revisión exhaustiva con evidencia de pruebas antes de continuar
4. **Continuar**: Avanzar solo tras aprobación explícita del usuario
...
- Seguir exactamente las 8 fases definidas en el plan (11 horas estimadas)
...
Proceso iterativo obligatorio siguiendo las 8 fases del plan de implementación:

**Estructura de Fases Obligatoria:**
- **FASE 0:** Setup del Proyecto (30 min) - pom.xml, configuración, Docker
...
- **Setup/Config:** Compilación + conexión BD + Docker funcional + comentarios de configuración + @EnableRetry
- **Migraciones:** Flyway exitoso + 4 tablas creadas (V1-V4) + datos iniciales (5 asesores) + comentarios SQL explicativos
- **Entities:** Hibernate valida schema + 4 entidades + relaciones correctas + JavaDoc completo
- **DTOs:** Validaciones Bean Validation + serialización JSON + 6 DTOs + comentarios de propósito
- **Repositories:** Queries ejecutan + resultados esperados + 4 repositories + documentación de métodos custom
- **Services:** Reglas de negocio RN-001 a RN-013 + transacciones + logging + 6 services + JavaDoc de lógica compleja
- **Controllers:** Endpoints responden + códigos HTTP correctos + 3 controllers + 13 endpoints + documentación de API
- **Schedulers:** Registro exitoso + ejecución programada + 2 schedulers + comentarios de frecuencia y propósito
...
6. **Comenzar FASE 0:** Iniciar con Setup del Proyecto siguiendo especificaciones exactas

**Primera Acción Requerida:**
Confirmar que has leído y comprendido el plan de implementación, luego comenzar con:

**FASE 0, PASO 0.1: Crear Estructura Base de Maven**

Implementar exactamente según las especificaciones del plan, validar compilación y funcionalidad, luego solicitar revisión usando el template obligatorio.

**Acción Final Obligatoria:**
Al completar TODAS las 8 fases, generar documentación técnica completa:
...
**¿Estás listo para comenzar la implementación del Sistema Ticketero?**

---

**Estado:** Refinado y Validado según Metodología Universal  
**Próximo Uso:** Listo para implementación completa del Sistema Ticketero en 8 fases
...
**FASE 0 y FASE 6:** Consultar `docs/architecture/diagrams/01-context-diagram.puml`
